@echo off
rem
rem Copyright 1991-2015 Mentor Graphics Corporation
rem
...
echo link /NODEFAULTLIB:libcmt.lib ucdbdump %PLATFORM_INSTALL_HOME%\libucdb.lib
link /NODEFAULTLIB:libcmt.lib ucdbdump %PLATFORM_INSTALL_HOME%\libucdb.lib
...
:exit_setup
	...
	if (%is64bit% == 1) (
		echo Make sure you have Microsoft Visual Studio 9.0 Professional edition installed with the necessary SDK's.
	) else (
