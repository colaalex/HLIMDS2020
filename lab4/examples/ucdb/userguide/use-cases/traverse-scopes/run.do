...
if {$tcl_platform(platform) eq "windows"} {
	...
	if {$is64bit == 1 } {
		quietly set RESFILE traverse_scopes64.res
	} else {
		quietly set RESFILE traverse_scopes32.res
	}
} else {
...
echo $CC -std=c99 traverse_scopes.c
eval $CC -std=c99 traverse_scopes.c
echo $LD traverse_scopes traverse_scopes.o $UCDBLIB $RESFILE
eval $LD traverse_scopes traverse_scopes.o $UCDBLIB $RESFILE

# Run the UCDB application.
echo traverse_scopes
traverse_scopes
