...
if {$tcl_platform(platform) eq "windows"} {
	...
	if {$is64bit == 1 } {
		quietly set RESFILE read_attrtags64.res
	} else {
		quietly set RESFILE read_attrtags32.res
	}
} else {
...
echo $CC -std=c99 read_attrtags.c
eval $CC -std=c99 read_attrtags.c
echo $LD read_attrtags read_attrtags.o $UCDBLIB $RESFILE
eval $LD read_attrtags read_attrtags.o $UCDBLIB $RESFILE
