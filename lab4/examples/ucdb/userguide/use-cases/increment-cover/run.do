...
if {$tcl_platform(platform) eq "windows"} {
	...
	if {$is64bit == 1 } {
		quietly set RESFILE increment_cover64.res
	} else {
		quietly set RESFILE increment_cover32.res
	}
} else {
...
echo $CC -std=c99 increment_cover.c
eval $CC -std=c99 increment_cover.c
echo $LD increment_cover increment_cover.o $UCDBLIB $RESFILE
eval $LD increment_cover increment_cover.o $UCDBLIB $RESFILE

# Run the UCDB application.
echo increment_cover
increment_cover
