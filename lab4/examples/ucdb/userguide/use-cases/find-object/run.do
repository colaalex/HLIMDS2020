...
if {$tcl_platform(platform) eq "windows"} {
	...
	if {$is64bit == 1 } {
		quietly set RESFILE find_object64.res
	} else {
		quietly set RESFILE find_object32.res
	}
} else {
...
if {$platform eq "sunos5v9"} {
	echo $CC find_object.c
	eval $CC find_object.c
} else {
	echo $CC -std=c99 find_object.c
	eval $CC -std=c99 find_object.c
}
echo $LD find_object find_object.o $UCDBLIB $RESFILE
eval $LD find_object find_object.o $UCDBLIB $RESFILE
