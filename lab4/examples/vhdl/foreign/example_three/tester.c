...
)
{
	...
	/* value is not null terminate because it isn't really a string */
	strncpy(this_testpoint->test_val, values, tester_ports[portnum].width);
    ...
    for ( cur_testpoint=testpoints; cur_testpoint;
        ...
        if ( cur_testpoint->type == DRIVE ) {
            ...
            if ( ! is_array ) {
                ...
                if ( sigval != (mtiLongT) *cur_testpoint->test_val ) {
                    sprintf( buf,
