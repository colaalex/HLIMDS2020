...
#            Run UCDB example ............... cygwin_mingwgcc.sh run
...
if [ "$1" != "run" ] ; then
    ...
    echo "### Run UCDB example ............... cygwin_mingwgcc.sh run"
...
if [ $is64bit -eq 1 ]; then
	UCDBRES=ucdbdump64.res
else
	UCDBRES=ucdbdump.res
fi

echo $LD ucdbdump ucdbdump.o $UCDBLIB $UCDBRES
$LD ucdbdump ucdbdump.o $UCDBLIB $UCDBRES
